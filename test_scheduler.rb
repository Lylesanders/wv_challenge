...
class TestScheduler < Minitest::Test
		...
		def test_master_student_list_Megan
			...
			assert_equal([["Smith","Megan","Coding","Robotics","EMT","Fire_fighting"]],current_schedule.master_student_list(file_name))
		end

		def test_master_student_list_Jeff
			current_schedule= Schedule.new
			file_name = "jeff_fox.csv"
			assert_equal([["Fox","Jeff","Fire_fighting","Coding","EMT","Robotics"]],current_schedule.master_student_list(file_name))
		end

		def test_master_student_list_Clarisa
			current_schedule= Schedule.new
			file_name = "Clarisa_Darling.csv"
			assert_equal([["Darling","Clarisa","coding","robotics","fire_fighting","emt"]],current_schedule.master_student_list(file_name))
		end

		def test_class_list
			current_schedule= Schedule.new
			class_file = "classes.csv"
			assert_equal([["Coding",15],["Robotics",25],["Fire_fighting",50],["EMT",30]],current_schedule.master_class_list(class_file))
		end

		def test_class_list_two
			current_schedule= Schedule.new
			class_file = "classb.csv"
			assert_equal([["Dish_washing",24],["Mining",13],["Culinary_arts",45],["Lawn_care",26]],current_schedule.master_class_list(class_file))
		end
		...
		def test_schedule_score_one
			current_schedule= Schedule.new
			scheduled =  [["Monk","Adrian","EMT",4],["Bundy","Al","EMT",2],["Keaton","Alex","Robotics",3],["McBeal","Ally","Coding",4],["Sipowicz","Andy","Fire_fighting",4]]
			assert_equal(17,current_schedule.weighted_schedule(scheduled))
		end

		def test_schedule_score_two
			current_schedule= Schedule.new
			scheduled =  [["Monk","Adrian","EMT",4],["Bundy","Al","EMT",4],["Keaton","Alex","Robotics",4],["McBeal","Ally","Coding",4],["Sipowicz","Andy","Fire_fighting",4]]
			assert_equal(20,current_schedule.weighted_schedule(scheduled))
		end

		def test_schedule_score_three
			current_schedule= Schedule.new
			scheduled =  [["Monk","Adrian","EMT",1], ["Bundy","Al","EMT",2], ["Keaton","Alex","EMT",3], ["McBeal","Ally","Coding",2],["Sipowicz","Andy","Robotics",2], ["Taylor","Andy","EMT",3], ["Marie","Ann","Coding",4],["Fonzereli","Author","Coding",4]]
			assert_equal(21,current_schedule.weighted_schedule(scheduled))
		end

		def test_schedule_score_three
			current_schedule= Schedule.new
			scheduled =  [["Pembleton","Frank","EMT",3], ["Crane","Frasier","Robotics",1], ["Sanford","Fred","Fire_fighting",2],["Cast","Freida","Coding",2], ["Jefferson","George","Coding",2], ["Grissom","Gil","EMT",2], ["Addams","Gomez","Fire_fighting",4], ["Munster","Herman","EMT",3], ["Simpson","Homer","Fire_fighting",4], ["Caine","Horatio","Coding",1]]
			assert_equal(24,current_schedule.weighted_schedule(scheduled))
		end
		def test_schedule_score_four
			current_schedule= Schedule.new
			scheduled =  ["Fever","Johnny","Robotics",2],["Bartlet","Josiah","EMT",2],["Defazio","Laverne","Fire_fighting",4],["Briscoe","Lennie","Coding",3],["Grant","Lou","Fire_fighting",1],["DePalma","Louie","Robotics",2],["Columbo","Lt.","EMT",1],["Kojak","Theo","Lt.",2],["Ricardo","Lucy","Coding",2]
			assert_equal(19,current_schedule.weighted_schedule(scheduled))
		end
		...
		def test_csv_file_two
			...
			scheduled = [["Ross","Doug","Robotics",3]]
			current_schedule.make_current_schedule(make_file, scheduled)
			actual_csv = File.open(make_file).read
		...
		def test_csv_file_four
			...
			scheduled = [["Wayne","Bruce","Fire_fighting",4], ["Kent","Clark","EMT",3]]
			current_schedule.make_current_schedule(make_file, scheduled)
			actual_csv = File.open(make_file).read
